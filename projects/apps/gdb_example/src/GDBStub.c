...
uint8_t read_unaligned_byte(uint8_t *addr) {
  uint32_t *base = (uint32_t *) ((int) addr & ~0x3);
  uint32_t word;

  word = *base;
  return (uint8_t)(word >> 8 * ((int) addr & 0x3));
}
...
void gdb_end_packet() {
	os_printf("#%c%c", hexdigit(gdb_send_checksum >> 4),
...
void gdb_exception_handler(struct xtos_saved_regs *frame) {
  ...
  os_memcpy(&regs.a[2], frame->a, sizeof(frame->a));
