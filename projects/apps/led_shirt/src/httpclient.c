...
static char * esp_strdup(const char * str)
{
	...
	char * new_str = (char *) pvPortMalloc(os_strlen(str) + 1); // 1 for null character
	if (new_str == NULL) {
...
static void ICACHE_FLASH_ATTR receive_callback(void * arg, char * buf, unsigned short len)
{
	...
	char * new_buffer;
	if (new_size > BUFFER_SIZE_MAX || NULL == (new_buffer = (char *)pvPortMalloc(new_size))) {
...
static void ICACHE_FLASH_ATTR connect_callback(void * arg)
{
	...
	char buf[120 + strlen(method) + strlen(req->path) + strlen(req->hostname) +
...
static void ICACHE_FLASH_ATTR dns_callback(const char * hostname, ip_addr_t * addr, void * arg)
{
	...
	if (addr == NULL) {
		...
		struct espconn * conn = (struct espconn *)pvPortMalloc(sizeof(struct espconn));
		conn->type = ESPCONN_TCP;
		conn->state = ESPCONN_NONE;
		conn->proto.tcp = (esp_tcp *)pvPortMalloc(sizeof(esp_tcp));
...
void ICACHE_FLASH_ATTR http_raw_request(const char * hostname, int port, bool secure, const char * path, const char * post_data, const char * headers, http_callback user_callback)
{
	...
	request_args * req = (request_args *)pvPortMalloc(sizeof(request_args));
	...
	req->buffer_size = 1;
	req->buffer = (char *)pvPortMalloc(1);
