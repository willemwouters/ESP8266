...
void ICACHE_FLASH_ATTR httpdSendHtmlHeaders(HttpdConnData *conn) {
	char buff[1024];
	int l;
	l=os_sprintf(buff, "<html>\r\n<head>\r\n<title>Config Wifi Ap</title>\r\n</head>\r\n<body>");
	espconn_sent(conn->conn, (uint8 *)buff, l);
}
void ICACHE_FLASH_ATTR httpdSendHtmlEndHeaders(HttpdConnData *conn) {
	char buff[1024];
	int l;
	l=os_sprintf(buff, "</body>\r\n</html>");
	espconn_sent(conn->conn, (uint8 *)buff, l);
}
