...
static TcpUdpDataCallback TcpCallback;
/**
...
#define IPADDR_MULTINOT  ((long) 0xF4000001UL) // 244.0.0.1
#define IPADDR_MULTI   ((long) 0xE0000001UL) // 224.0.0.1
#define IPADDR_ANY          ((long) 0x00000000UL) // 0.0.0.0
#define IPADDR_OWNIP     ((long)0xC0A80401UL) // 192.168.4.1

err_t SendUdpData(char * data) {
	  err_t err = espconn_sent(pUdpServer, (uint8_t *) data, os_strlen(data));
	  return err;
}
/**
  ...
  */
void ICACHE_FLASH_ATTR
SetupTcpUdpServer(int enable, int port, int type, TcpUdpDataCallback clb)
{
    ...
    if(type != 2) {
      ...
      if(type != 1) {
        ...
        pUdpServer->reverse = NULL;
       	...
       	pUdpServer->proto.udp->remote_ip[0] = 239;
       	pUdpServer->proto.udp->remote_ip[1] = 0;
       	pUdpServer->proto.udp->remote_ip[2] = 0;
       	pUdpServer->proto.udp->remote_ip[3] = 1;
        espconn_regist_recvcb(pUdpServer, at_udpserver_recv);
        espconn_create(pUdpServer);
        espconn_accept(pUdpServer);

        ip_addr_t iphost;
        iphost.addr = IPADDR_ANY;

        ip_addr_t ipt;
       	IP4_ADDR(&ipt, 239, 0, 0, 1);


        int i = espconn_igmp_join(&iphost,  &ipt);
        os_printf("-%s-%s UdpServer success: %d port: %d \r\n", __FILE__, __func__, i, (int) port);
      }
  }
  else
  {
